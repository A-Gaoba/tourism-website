...
export default function HotelsPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8 mt-16 rtl">
      <h1 className="text-3xl font-bold mb-8 text-right">الفنادق والشاليهات</h1>
      <CityFilter cities={cities} selectedCity={selectedCity} onSelectCity={setSelectedCity} />
      <Tabs
        dir="rtl"
        value={accommodationType}
        onValueChange={(value) => setAccommodationType(value as "hotels" | "cottages")}
        className="mt-4"
      >
        <TabsList className="w-42 neumorphic-shadow dark:bg-white">
          <TabsTrigger
            value="cottages"
            className="w-1/2 neumorphic-shadow data-[state=active]:neumorphic-inset transition-all duration-300"
          >
            الشاليهات
          </TabsTrigger>
          <TabsTrigger
